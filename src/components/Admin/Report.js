...
const Report = () => {
  ...
  const fetchProductData = async () => {
    try {
      const response = await axios.get(
        "https://ecommerce-coolmate-server-production.up.railway.app/api/admin/report/product",
        {
  ...
  const fetchCustomerData = async () => {
    try {
      const response = await axios.get(
        "https://ecommerce-coolmate-server-production.up.railway.app/api/admin/report/customer",
        {
          withCredentials: true,
        }
      );
      ...
      if (response.status === 200 && response.data.succes) {
        ...
        console.error("Lỗi khi lấy dữ liệu khách hàng:", response.data.message);
      }
    } catch (error) {
