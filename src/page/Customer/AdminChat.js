...
const socket = io(
  "https://ecommerce-coolmate-server-production.up.railway.app",
  {
...
const AdminChat = () => {
  ...
  const loadRooms = async () => {
    try {
      const response = await axios.get(
        "https://ecommerce-coolmate-server-production.up.railway.app/api/customer/messenger",
        {
          withCredentials: true,
        }
      );
  ...
  const loadMessages = async (currentRoom) => {
    try {
      const response = await axios.get(
        `https://ecommerce-coolmate-server-production.up.railway.app/api/customer/messenger/${currentRoom.id}?senderId=4`,
        {
          withCredentials: true,
        }
      );
    ...
    if (message.trim()) {
      ...
      try {
        const response = await axios.post(
          `https://ecommerce-coolmate-server-production.up.railway.app/api/customer/messenger/${currentRoom.id}`,
          newMessage,
          {
            withCredentials: true,
            headers: {
            ...
            {rooms.map((room) => (
              <div
                ...
                className={`flex items-center cursor-pointer p-2 rounded-md ${
                  room.id === currentRoom ? "bg-gray-200" : "bg-transparent"
                }`}
                onClick={() => {
                  setCurrentRoom(room);
                }}
              ...
              <>
                <div className="flex pb-2">
                  <img
                    ...
                    className="w-10 h-10 rounded-full mr-2"
                  />
                  <div className="text-xl ">{currentRoom.fullName}</div>
                </div>
              </>
            ) : (
              <>
                <div className="text-xl pb-4 font-bold">Chọn phòng chat</div>
              </>
            )}
          </h3>
          <div
                  ...
                  >
                    ...
                    <div className="relative">
                      <div
                        ...
                        style={{
                          marginLeft:
                            isUserMessage && isLastUserMessage ? "0" : "40px",
                        }}
                      >
